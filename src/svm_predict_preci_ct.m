%对降雨进行预测，svm_ct
%SVM, 不分每月进行预测------------
%支持向量回归进行预测每月的降雨, 自变量为气候指标（最相关的两个），前1个月降雨，最小温度，最大温度，平均温度，分季节进行预测
clear all;
clc;
%1960-2016
...
[m2 n2] = size(preci);
svm_ct_predict = NaN(m2,n2,lead);%行*列*lead  提前1-8个月的预测值
num_select = 2;%前2个最相关的气候指标
svm_ct_para_save = NaN(n, lead, 3); % C, epsilon, kernelscale
for i=1:n
    for j=1:lead
...
%         x_pre = [all_climate_index(1:end, :)];
        ...
        Mdl = fitrsvm(train_sub, label_sub,'KernelFunction','rbf','KernelScale','auto','Standardize',true);
        svm_ct_para_save(i, j, 1) = Mdl.BoxConstraints(1);
        svm_ct_para_save(i, j, 2) = Mdl.Epsilon;
        svm_ct_para_save(i, j, 3) = Mdl.KernelParameters.Scale;
...
%         end
        ...
        svm_ct_predict(:, i, j) = ynew; %第i个点提前第j个月的预测
    end
    disp(i);
end

save('D:\干旱\data\result8\svm_predict_p_ct.mat','svm_ct_predict');
disp('finished');
