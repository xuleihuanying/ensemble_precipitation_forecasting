...
[m2 n2] = size(preci);
statistical_svm_predict = NaN(m2,n2,lead);%行*列*lead  提前1-8个月的预测值
svm_para_save = NaN(n, lead, 3); % C, epsilon, kernelscale
for i=1:n
    for j=1:lead
...
%             statistical_svm_predict(month:12:end, i, j) = ynew; %第i个点提前第j个月的预测
%         end
...
%         end
        ...
        statistical_svm_predict(:, i, j) = ynew; %第i个点提前第j个月的预测
    end
    disp(i);
end

save('D:\干旱\data\result8\svm_predict_p.mat','statistical_svm_predict');
disp('finished');
% save('D:\干旱\data\result3\svm_para_save.mat','svm_para_save');