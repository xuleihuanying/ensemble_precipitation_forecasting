%对降雨进行预测，rf_ct, 选择所有的变量，相当于bagging，只对样本采样进行随机
%RF, 不分每月进行预测------------
clear all;
clc;
%1960-2016
...
[m2 n2] = size(preci);
rf_ct_predict = NaN(m2,n2,lead);%行*列*lead  提前1-8个月的预测值
num_select = 2;%前2个最相关的气候指标
tree_num = 20; % tree number
tree_v_sct = 5; % selected number of predictors per run
for i=1:n
    for j=1:lead
...
%         end
        ...
        rf_ct_predict(:, i, j) = ynew; %第i个点提前第j个月的预测
    end
    disp(i);
end

save('D:\干旱\data\result8\rf_predict_p_ct.mat','rf_ct_predict');
disp('finished');
