...
# lstm, temperature & climate index
...
for i in range(n):
    for j in range(lead):
        ...
        x2 = np.concatenate([np.zeros((j + 1), dtype=np.float), tem[0:train_end - j - 1, i]], axis=0)  # 提前1个月降雨
        x3 = np.concatenate([np.zeros((j + 1), dtype=np.float), tem_min[0:train_end - j - 1, i]], axis=0)  # 提前1个月降雨
        x4 = np.concatenate([np.zeros((j + 1), dtype=np.float), tem_max[0:train_end - j - 1, i]], axis=0)  # 提前1个月降雨
        x5 = np.concatenate([np.zeros((j + 1, ci_col), dtype=np.float), all_climate_index[0:train_end-j-1, :]], axis=0)
        ...
        x5 = np.reshape(x5, (train_end, ci_col), order='F')

        train_sub = np.concatenate([x1, x2, x3, x4, x5], axis=1)
        ...
        x2 = np.concatenate([np.zeros((j + 1), dtype=np.float), tem[0:all_end - j - 1, i]], axis=0)  # 提前1个月降雨
        x3 = np.concatenate([np.zeros((j + 1), dtype=np.float), tem_min[0:all_end - j - 1, i]], axis=0)  # 提前1个月降雨
        x4 = np.concatenate([np.zeros((j + 1), dtype=np.float), tem_max[0:all_end - j - 1, i]], axis=0)  # 提前1个月降雨
        x5 = np.concatenate([np.zeros((j + 1, ci_col), dtype=np.float), all_climate_index[0:all_end - j - 1, :]],axis=0)
        ...
        x5 = np.reshape(x5, (all_end, ci_col), order='F')

        x_pre = np.concatenate([x1, x2, x3, x4, x5], axis=1)
...
np.savetxt('D:/干旱/data/result8/lstm_predict_p_ct.txt', lstm_predict)
